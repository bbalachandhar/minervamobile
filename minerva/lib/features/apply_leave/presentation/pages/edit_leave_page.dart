// minerva/lib/features/apply_leave/presentation/pages/edit_leave_page.dart
import 'dart:io';
...
class EditLeavePage extends StatefulWidget {
  ...
  const EditLeavePage({Key? key, required this.leaveApplication}) : super(key: key);
...
class _EditLeavePageState extends State<EditLeavePage> {
  ...
  //String? _initialDocumentUrl; // To show existing document if any
  ...
  void _populateFields() {
    _applyDateController.text = widget.leaveApplication.applyDate;
    _fromDateController.text = widget.leaveApplication.fromDate;
    _toDateController.text = widget.leaveApplication.toDate;
    ...
    if (widget.leaveApplication.docs != null && widget.leaveApplication.docs!.isNotEmpty) {
      _selectedFileName = widget.leaveApplication.docs!;
      // _initialDocumentUrl = '${imagesUrl}uploads/student_leavedocuments/${widget.leaveApplication.docs}';
      // For editing, we don't load the file itself, just display its name
    }
  }
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      context.read<ApplyLeaveBloc>().add(EditLeaveApplication(
            leaveId: widget.leaveApplication.id,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Leave'), // editleaveheading
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text(
                'Edit Leave Application', // Matches @string/editleaveheading
                style: TextStyle(
                  fontSize: 22.0,
  ...
  Widget _buildFilePicker() {
    return Column(
      ...
      children: [
        ...
        if (_selectedFile != null || _selectedFileName.isNotEmpty)
          Padding(
                ...
                IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      _selectedFile = null;
                      _selectedFileName = '';
                      // _initialDocumentUrl = null; // Clear initial URL as well if needed
                    });
                  },
                ),
              ],
